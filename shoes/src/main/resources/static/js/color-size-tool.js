...
document.querySelector("#addRow").addEventListener("click", () => {
  ...
  newRow += '<div class="col-sm-2"> ';
  newRow +=
    '<button type="button" class="removeRow btn btn-outline-danger" tabindex="-1"><i class="fa-solid fa-xmark"></i></button>';
  newRow += "</div>";
  colorCount += 1;
...
document.querySelector("#newRow").addEventListener("click", (e) => {
  if (
    (e.target.matches(".removeRow") || e.target.matches("svg") || e.target.matches("path")) &&
    confirm("색상 정보를 삭제하시겠습니까?")
  ) {
...
document.querySelector(".btn-success").addEventListener("click", (e) => {
  //최소 한가지 이상의 색/사이즈 입력했는지
  if (document.querySelector("#color-necessary").value == "" || document.querySelector("#size-necessary").value == "") {
    alert("필수 색상과 사이즈를 입력해야 합니다.");
    return;
  }
  if (
    tbody.innerHTML != "" &&
    !confirm("주의 : 재고 입력칸을 재생성하면 기존 재고 입력값이 사라집니다. 그래도 진행할까요?")
  ) {
    return;
  }
  ...
  colorList.forEach((color) => {
    ...
    sizesSplit.forEach((size, sizeidx) => {
      ...
      tr +=
        '<td><input type="text" class="form-control form-control-sm text-center align-middle" name="quantityList[' +
        idx +
        ']" value="0"/></td>';
      tr += "</tr>";
